...
The `Table` struct supports: `StructName` (required, matches the Go struct name), `Name` (table name), `SearchPath`, `Comment`, `Options` (e.g. `WITH (fillfactor=70)`), `Type` (`"TEMPORARY"`), `Partitioned` (`&mirage.Partition{Strategy:"RANGE", Columns:[]string{"created_at"}}` for `PARTITION BY`), `PartitionOf` (`&mirage.PartitionBound{ParentTable:"events", Bounds:"FOR VALUES FROM ('2024-01-01') TO ('2025-01-01')"}` for `PARTITION OF`), and `Ignore` (skip in migration generation).
