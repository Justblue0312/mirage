...
// ActiveUsers is a view of non-deleted users - v5 tightens filter to role=member.
var _ = mirage.Register(mirage.View{
	Name:        "active_users",
	Description: "Users that are not suspended or deleted (v5: members only)",
	Query:       "SELECT id, username, email, role, created_at FROM users WHERE status = 'active' AND role='member'",
})
