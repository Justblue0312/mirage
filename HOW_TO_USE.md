...
```go
func init() {
    mirage.Register(mirage.Table{StructName: "Event", Name: "events", Partitioned: &mirage.Partition{Strategy: "RANGE", Columns: []string{"created_at"}}})
}
...
func init() {
    mirage.Register(mirage.Table{
        ...
        Name:       "events_2024_q1",
        PartitionOf: &mirage.PartitionBound{ParentTable: "events", Bounds: "FOR VALUES FROM ('2024-01-01') TO ('2024-04-01')"},
    })
}
...
### JSON Columns (struct / slice / map as jsonb)

Store any Go struct, `*Foo`, `[]Foo`, `*[]Foo`, `map[string]any` as `jsonb`. The inner struct **must** have `json:"..."` tags (otherwise Go 1.26+ `encoding/json` omits untagged fields):
...
type Bar struct {
    ID  int64 `db:"pk,identity,type=bigserial"`
    Foo Foo   `db:"name=foo,type=jsonb,notnull,default='{}'"`
    Items []Foo `db:"name=items,type=jsonb,null"`
    Extra *Foo  `db:"name=extra,type=jsonb,null"`
}
```
`type=jsonb` is preferred; `type=json` is honored as `json`. Without an explicit `type`, a bare struct field infers `jsonb`.
