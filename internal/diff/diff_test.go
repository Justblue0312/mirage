...
func TestDiff(t *testing.T) {
	...
	t.Run("empty to table with non-unique indexes yields TableAdded plus IndexAdded per index", func(t *testing.T) {
		oldPkg := &schema.Package{}
		newPkg := &schema.Package{
			Tables: []schema.Table{
				{
		...
		var indexEvents []EventKind
		var tableEvent bool
		for _, e := range events {
			if e.Kind == TableAdded {
				tableEvent = true
			}
			if e.Kind == IndexAdded {
				indexEvents = append(indexEvents, e.Kind)
			}
		}
		if !tableEvent {
			t.Error("expected a TableAdded event")
		}
		if len(indexEvents) != 2 {
			t.Fatalf("expected 2 IndexAdded events for new table indexes, got %d: %v", len(indexEvents), events)
		}
	})
