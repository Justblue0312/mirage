...
const (
	...
	PartitionChanged        EventKind = "partition_changed"
	PartitionBoundChanged   EventKind = "partition_bound_changed"
...
func Diff(old, new *schema.Package) []DeltaEvent {
	...
	for _, name := range sortedKeys(newTables) {
		...
		if !ok {
			...
			// Unique constraints, FK, and check constraints are emitted as
			// part of the CREATE TABLE statement by the generator, so we must
			// NOT also emit separate UniqueAdded/FKAdded/CheckAdded events
			// here (that would create them twice). Non-unique indexes are NOT
			// rendered by CREATE TABLE, so emit an IndexAdded event per index
			// so the generator can emit them after the table.
...
func diffTables(old, new *schema.Table) []DeltaEvent {
	...
	if !old.Partitioned.Equal(new.Partitioned) {
		events = append(events, DeltaEvent{
	...
	if !old.PartitionOf.Equal(new.PartitionOf) {
		events = append(events, DeltaEvent{
			Kind:     PartitionBoundChanged,
			Table:    name,
			Severity: Warning,
			OldTable: old,
			NewTable: new,
