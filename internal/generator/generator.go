...
func (g *Generator) Generate() (*MigrationFile, error) {
	...
	partitionChanges := g.filterEvents(diff.PartitionChanged)
	partitionBoundChanges := g.filterEvents(diff.PartitionBoundChanged)
	...
	// Partition changes require table rebuild; emit detached partitions as best-effort.
	for _, e := range partitionChanges {
		if e.NewTable != nil && e.OldTable != nil {
			// Parent partition strategy/columns changed — Postgres cannot ALTER PARTITION BY in place.
			upStmts = append(upStmts, fmt.Sprintf("-- partition change on %s: requires manual migration (drop/recreate)", e.Table))
			upStmts = append(upStmts, fmt.Sprintf("--   from PARTITION BY %s (%s)", e.OldTable.Partitioned.Strategy, strings.Join(e.OldTable.Partitioned.ColumnsList(), ", ")))
			upStmts = append(upStmts, fmt.Sprintf("--   to   PARTITION BY %s (%s)", e.NewTable.Partitioned.Strategy, strings.Join(e.NewTable.Partitioned.ColumnsList(), ", ")))
		} else if e.NewTable != nil && e.NewTable.Partitioned != nil {
			upStmts = append(upStmts, fmt.Sprintf("-- partition added on %s: PARTITION BY %s (%s)", e.Table, e.NewTable.Partitioned.Strategy, strings.Join(e.NewTable.Partitioned.ColumnsList(), ", ")))
		}
	}
	for _, e := range partitionBoundChanges {
		if e.NewTable != nil && e.NewTable.PartitionOf != nil {
			// Child partition bound changed — drop and recreate child.
			if e.OldTable != nil && e.OldTable.PartitionOf != nil {
				upStmts = append(upStmts, g.dialect.DropTable(*e.OldTable)...)
			}
			upStmts = append(upStmts, g.dialect.CreateTable(*e.NewTable)...)
		} else if e.OldTable != nil && e.OldTable.PartitionOf != nil {
			upStmts = append(upStmts, g.dialect.DropTable(*e.OldTable)...)
		}
	}
...
func (g *Generator) generateDown() ([]string, error) {
	...
	for _, e := range reversed {
		...
		case diff.ExtensionAdded:
			if e.NewExtension != nil {
				extensionDropStmts = append(extensionDropStmts, g.dialect.DropExtension(*e.NewExtension))
			}
		...
		case diff.PartitionChanged:
			if e.OldTable != nil {
				stmts = append(stmts, fmt.Sprintf("-- revert partition change on %s", e.Table))
				// Best-effort: recreate old parent if needed
				if e.OldTable.Partitioned != nil {
					stmts = append(stmts, fmt.Sprintf("--   revert to PARTITION BY %s (%s)", e.OldTable.Partitioned.Strategy, strings.Join(e.OldTable.Partitioned.ColumnsList(), ", ")))
				}
			}
		case diff.PartitionBoundChanged:
			if e.NewTable != nil && e.NewTable.PartitionOf != nil {
				// Was added on up, so drop on down
				stmts = append(stmts, g.dialect.DropTable(*e.NewTable)...)
			} else if e.OldTable != nil && e.OldTable.PartitionOf != nil {
				stmts = append(stmts, g.dialect.CreateTable(*e.OldTable)...)
			}
		}
	}
