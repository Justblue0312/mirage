...
func TestGenerate_ExtensionDropOrderedAfterTableDrops(t *testing.T) {
	d := postgres.New()
	ext := schema.Extension{Name: "citext", IfNotExists: true}
	tbl := schema.Table{
		StructName: "User",
		Name:       "users",
		Columns: []*schema.Column{
			{Name: "id", SQLType: "bigint", Nullable: false},
			{Name: "email", SQLType: "citext", Nullable: false},
		},
	}
	oldPkg := helperNewPkg(nil, nil)
	newPkg := &schema.Package{Extensions: []schema.Extension{ext}, Tables: []schema.Table{tbl}}
	...
	lastIdx := len(mf.DownStatements) - 1
	if !strings.Contains(mf.DownStatements[lastIdx], "DROP EXTENSION IF EXISTS \"citext\"") {
		t.Errorf("last down should be DROP EXTENSION (after table drops), got: %v", mf.DownStatements)
	}
	for i, stmt := range mf.DownStatements {
		if strings.Contains(stmt, "DROP TABLE") {
			if i >= lastIdx {
				t.Errorf("DROP EXTENSION at index %d must follow DROP TABLE at index %d, got: %v", lastIdx, i, mf.DownStatements)
			}
			break
		}
	}
