...
func (p *Postgres) CreateTable(t schema.Table) []string {
	...
	// Child partitions use PARTITION OF syntax; they ignore colDefs body (inherited).
	if t.PartitionOf != nil {
		bounds := strings.TrimSpace(t.PartitionOf.Bounds)
		if bounds != "" && !strings.HasPrefix(strings.ToUpper(bounds), "FOR VALUES") {
			bounds = "FOR VALUES " + bounds
		}
		if bounds == "" {
			bounds = "DEFAULT"
		}
		stmt := fmt.Sprintf("CREATE TABLE %s PARTITION OF %s %s;", t.SQLName(), p.QuoteIdent(t.PartitionOf.ParentTable), bounds)
		var stmts []string
	...
	if t.Partitioned != nil {
		cols := t.Partitioned.ColumnsList()
		if len(cols) > 0 {
			quoted := make([]string, len(cols))
			for i, c := range cols {
				quoted[i] = p.QuoteIdent(c)
			}
			partition = fmt.Sprintf(" PARTITION BY %s (%s)", t.Partitioned.Strategy, strings.Join(quoted, ", "))
		}
	}
