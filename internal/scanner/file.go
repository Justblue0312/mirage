...
func buildAttrsFromTable(tc *schema.Table) Attrs {
	...
	if tc.Partitioned != nil {
		cols := tc.Partitioned.ColumnsList()
		args := append([]string{tc.Partitioned.Strategy}, cols...)
		attrs["partitioned"] = AttrValue{Args: args}
	}
	if tc.PartitionOf != nil {
		attrs["partition_of"] = AttrValue{Args: []string{tc.PartitionOf.ParentTable, tc.PartitionOf.Bounds}}
	}
	if len(tc.Uniques) > 0 {
		var groups [][]string
...
func parseTableLiteral(cl *ast.CompositeLit) *schema.Table {
	...
	for _, elt := range cl.Elts {
		...
		case "PartitionOf":
			tc.PartitionOf = parsePartitionBoundLiteral(kv.Value)
		case "Uniques":
...
func parsePartitionLiteral(expr ast.Expr) *schema.Partition {
	...
	for _, elt := range cl.Elts {
		...
		case "Columns":
			p.Columns = parseStringSliceLit(kv.Value)
		}
	}
	if len(p.Columns) == 0 && p.Column != "" {
		p.Columns = []string{p.Column}
	}
	return p
}

func parsePartitionBoundLiteral(expr ast.Expr) *schema.PartitionBound {
	if u, ok := expr.(*ast.UnaryExpr); ok && u.Op == token.AND {
		expr = u.X
	}
	cl, ok := expr.(*ast.CompositeLit)
	...
	for _, elt := range cl.Elts {
		...
		switch key.Name {
		case "ParentTable":
			pb.ParentTable = extractStringLit(kv.Value)
		case "Parent":
			pb.ParentTable = extractStringLit(kv.Value)
		case "Bounds":
			pb.Bounds = extractStringLit(kv.Value)
		case "Expression":
			pb.Bounds = extractStringLit(kv.Value)
		}
	}
