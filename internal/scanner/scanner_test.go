...
func TestGetStructTag(t *testing.T) {
	tests := []struct {
		...
		{
			name: "db tag with escaped backslash unescaped",
			src:  "type Foo struct { Phone string `db:\"name=phone_number,check=phone_number IS NULL OR phone_number ~* '^\\\\+?[0-9]{7,15}$'\"` }",
			tag:  "db",
			want: "name=phone_number,check=phone_number IS NULL OR phone_number ~* '^\\+?[0-9]{7,15}$'",
		},
	}
...
// TestParseColumnTag_QuotedCheckWithComma pins the phone-check regression: a
// comma inside a single-quoted regex must not split the tag, and the raw-tag
// backslash `\\` must unescape to `\` so the final check expression is the
// complete `'^\+?[0-9]{7,15}$'` (as rendered by the generator into DDL).
func TestParseColumnTag_QuotedCheckWithComma(t *testing.T) {
	src := "type Foo struct { Phone string `db:\"name=phone_number,type=text,null,check=phone_number IS NULL OR phone_number ~* '^\\\\+?[0-9]{7,15}$'\"` }"
	fset := token.NewFileSet()
	node, err := parser.ParseFile(fset, "test.go", "package test\n"+src, 0)
	if err != nil {
		t.Fatalf("parse error: %v", err)
	...
	for _, decl := range node.Decls {
		...
		for _, spec := range gen.Specs {
			...
			for _, f := range st.Fields.List {
				if fieldTag = getStructTag(f, "db"); fieldTag != "" {
					break
				}
			}
		}
	...
	if fieldTag == "" {
		t.Fatal("db tag not found")
	}

	col := ParseColumnTag(fieldTag)

	if col.Name != "phone_number" {
		t.Errorf("Name = %q, want phone_number", col.Name)
	}
	if col.SQLType != "text" {
		t.Errorf("SQLType = %q, want text", col.SQLType)
	}
	if col.Check != "phone_number IS NULL OR phone_number ~* '^\\+?[0-9]{7,15}$'" {
		t.Errorf("Check = %q, want phone_number IS NULL OR phone_number ~* '^\\+?[0-9]{7,15}$'", col.Check)
	}
}

// TestSplitTagParts_QuoteAware pins the quote-aware comma splitting in
// splitTagParts: commas inside single-quoted values or parentheses do not
// terminate the current part.
func TestSplitTagParts_QuoteAware(t *testing.T) {
	tests := []struct {
		name string
		raw  string
		want []string
		...
		{
			name: "comma inside single-quoted regex does not split",
			raw:  "check=phone_number IS NULL OR phone_number ~* '^\\+?[0-9]{7,15}$',notnull",
			want: []string{"check=phone_number IS NULL OR phone_number ~* '^\\+?[0-9]{7,15}$'", "notnull"},
		},
		{
			name: "comma inside parens does not split",
			raw:  "type=varchar(50,0),notnull",
			want: []string{"type=varchar(50,0)", "notnull"},
		},
		{
			name: "bare commas still split",
			raw:  "pk,notnull,type=bigint",
			want: []string{"pk", "notnull", "type=bigint"},
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := splitTagParts(tt.raw)
			if len(got) != len(tt.want) {
				t.Fatalf("splitTagParts() = %v, want %v", got, tt.want)
			}
			for i := range got {
				if got[i] != tt.want[i] {
					t.Errorf("splitTagParts()[%d] = %q, want %q", i, got[i], tt.want[i])
				}
			}
		})
	}
...
// TestSplitTagArgs_QuoteAware pins the quote-aware comma splitting in
// splitTagArgs: a comma inside a single-quoted value is preserved while
// comma-separated arguments are still split.
func TestSplitTagArgs_QuoteAware(t *testing.T) {
	args := splitTagArgs("phone_number IS NULL OR phone_number ~* '^\\+?[0-9]{7,15}$',failed_login_count >= 0")
	want := []string{"phone_number IS NULL OR phone_number ~* '^\\+?[0-9]{7,15}$'", "failed_login_count >= 0"}

	if len(args) != len(want) {
		t.Fatalf("splitTagArgs() = %v, want %v", args, want)
	}
	for i := range args {
		if args[i] != want[i] {
			t.Errorf("splitTagArgs()[%d] = %q, want %q", i, args[i], want[i])
		}
	}
}
