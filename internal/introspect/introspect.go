...
func FromLiveDatabase(ctx context.Context, pool *pgxpool.Pool, searchPath string) (*schema.Package, error) {
	...
	if err := introspectPartitions(ctx, pool, searchPath, pkg); err != nil {
		return nil, fmt.Errorf("introspecting partitions: %w", err)
	}
...
// introspectPartitions reads partition metadata from pg_partitioned_table and pg_inherits.
func introspectPartitions(ctx context.Context, pool *pgxpool.Pool, searchPath string, pkg *schema.Package) error {
	// Parent partitions: PARTITION BY
	parentQuery := `SELECT c.relname AS table_name, pt.partstrat, ARRAY_AGG(a.attname ORDER BY u.ordinality) AS columns
	FROM pg_class c
	JOIN pg_namespace n ON c.relnamespace = n.oid
	JOIN pg_partitioned_table pt ON c.oid = pt.partrelid
	JOIN unnest(pt.partattrs) WITH ORDINALITY u(attnum, ordinality) ON true
	JOIN pg_attribute a ON a.attrelid = c.oid AND a.attnum = u.attnum
	WHERE n.nspname = $1
	GROUP BY c.relname, pt.partstrat`
	rows, err := pool.Query(ctx, parentQuery, searchPath)
	if err != nil {
		return err
	}
	defer rows.Close()
	for rows.Next() {
		var tableName, partStrat string
		var cols []string
		if err := rows.Scan(&tableName, &partStrat, &cols); err != nil {
			return err
		}
		strategy := map[string]string{"r": "RANGE", "l": "LIST", "h": "HASH"}[partStrat]
		if strategy == "" {
			strategy = partStrat
		}
		if idx := findTable(pkg.Tables, tableName); idx >= 0 {
	...
	childQuery := `SELECT child.relname AS child_name, parent.relname AS parent_name, pg_get_expr(child.relpartbound, child.oid) AS bounds
	FROM pg_inherits
	...
	crows, err := pool.Query(ctx, childQuery, searchPath)
	if err != nil {
		return err
	}
	defer crows.Close()
	for crows.Next() {
		var childName, parentName, bounds string
		if err := crows.Scan(&childName, &parentName, &bounds); err != nil {
			return err
		}
		if idx := findTable(pkg.Tables, childName); idx >= 0 {
			pkg.Tables[idx].PartitionOf = &schema.PartitionBound{
