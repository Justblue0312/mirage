		...
		Action: func(ctx context.Context, cmd *cli.Command) error {
			...
			if connStr != "" {
				...
				for rows.Next() {
					...
					if err := rows.Scan(&version, &name, &appliedAt, &state); err != nil {
						return fmt.Errorf("scanning status: %w", err)
					}
					verStr := fmt.Sprintf("%d", version)
					applied[verStr] = StatusEntry{
						Version:   verStr,
						Name:      name,
						AppliedAt: appliedAt.Format(time.RFC3339),
						State:     state,
					}
					// also store padded form for compatibility with canonVersion
					applied[fmt.Sprintf("%014d", version)] = applied[verStr]
				}
